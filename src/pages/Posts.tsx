...
// Manages the top section with stats and the "New Post" button.
const PostHeader = ({ onCreatePost, postsCount }) => {
  return (
...
const PostsCalendar = ({ recentPosts }) => {
  ...
  const formatDateForComparison = (date) => `${date.getDate().toString().padStart(2, '0')}/${(date.getMonth() + 1).toString().padStart(2, '0')}/${date.getFullYear()}`;
  ...
  const renderCalendar = () => {
    ...
    const days = [];
    for (let i = 0; i < firstDay; i++) { days.push(<div key={`empty-${i}`} className="h-16 sm:h-20 md:h-24 border border-gray-100"></div>); }
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div key={day} className={`h-16 sm:h-20 md:h-24 border border-gray-100 p-1 sm:p-2 cursor-pointer transition-all hover:bg-blue-50 ${isToday ? 'bg-blue-100 border-blue-300' : 'bg-white'} ${selectedDate === day ? 'ring-2 ring-blue-500' : ''}`} onClick={() => setSelectedDate(selectedDate === day ? null : day)}>
          ...
          <div className="space-y-0.5 sm:space-y-1">{postsForDay.slice(0, window.innerWidth < 640 ? 1 : 2).map((post, index) => (<div key={post.id} className={`text-xs px-1 sm:px-2 py-0.5 sm:py-1 rounded-md sm:rounded-lg truncate ${post.status === 'published' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-yellow-100 text-yellow-700 border border-yellow-200'}`} title={post.title}>{post.title}</div>))}{postsForDay.length > (window.innerWidth < 640 ? 1 : 2) && <div className="text-xs text-gray-500 font-medium">+{postsForDay.length - (window.innerWidth < 640 ? 1 : 2)} more</div>}</div>
        </div>
      );
    }
    return days;
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div><h2 className="text-xl sm:text-2xl font-bold">{monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}</h2><p className="text-blue-100 mt-1 text-sm sm:text-base">Click on dates to view posts</p></div>
          <div className="flex items-center gap-2"><button onClick={() => navigateMonth(-1)} className="p-2 hover:bg-white/20 rounded-xl transition-colors"><ChevronLeft size={20} /></button><button onClick={() => setCurrentDate(new Date())} className="px-3 sm:px-4 py-2 bg-white/20 hover:bg-white/30 rounded-xl transition-colors text-xs sm:text-sm font-medium">Today</button><button onClick={() => navigateMonth(1)} className="p-2 hover:bg-white/20 rounded-xl transition-colors"><ChevronRight size={20} /></button></div>
        </div>
      </div>
      <div className="p-3 sm:p-6">
        <div className="grid grid-cols-7 gap-0 mb-4">{dayNames.map(day => (<div key={day} className="text-center text-xs sm:text-sm font-semibold text-gray-600 py-2"><span className="hidden sm:inline">{day}</span><span className="sm:hidden">{day.slice(0, 1)}</span></div>))}</div>
        <div className="grid grid-cols-7 gap-0 border border-gray-200 rounded-xl overflow-hidden">{renderCalendar()}</div>
      </div>
      {selectedDate && selectedDatePosts.length > 0 && (
        <div className="border-t border-gray-200 bg-gray-50 p-4 sm:p-6">
          <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2 text-sm sm:text-base"><Calendar size={18} className="text-blue-600" />Posts for {monthNames[currentDate.getMonth()]} {selectedDate}, {currentDate.getFullYear()}</h3>
          <div className="space-y-3">{selectedDatePosts.map(post => (<div key={post.id} className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200 hover:shadow-md transition-shadow"><div className="flex items-start justify-between gap-3"><div className="flex-1 min-w-0"><h4 className="font-semibold text-gray-900 mb-1 text-sm sm:text-base truncate">{post.title}</h4><p className="text-xs sm:text-sm text-gray-600 mb-2 line-clamp-2">{post.post}</p><div className={`inline-flex items-center gap-1 px-2 sm:px-3 py-1 rounded-full text-xs font-medium ${post.status === 'published' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-yellow-100 text-yellow-700 border border-yellow-200'}`}>{post.status === 'published' ? (<><CheckCircle2 size={12} /><span className="hidden sm:inline">Published</span><span className="sm:hidden">Pub</span></>) : (<><Clock3 size={12} /><span>Draft</span></>)}</div></div><div className="flex items-center gap-1 sm:gap-2 flex-shrink-0"><button className="p-1.5 sm:p-2 hover:bg-gray-100 rounded-lg transition-colors"><Eye size={14} className="text-gray-400" /></button><button className="p-1.5 sm:p-2 hover:bg-gray-100 rounded-lg transition-colors"><Edit3 size={14} className="text-gray-400" /></button></div></div></div>))}</div>
        </div>
      )}
    </div>
  );
...
const Posts = () => {
  ...
  // Use the imported PostFormData type for type safety
  const handleSubmitPost = (formData: PostFormData) => {
    ...
    const newPost = {
      ...
      date: formattedDate,
      status: "draft", // New posts are always drafts initially
      post: formData.prompt,
    };

    setRecentPosts(prev => [newPost, ...prev].sort((a, b) => new Date(b.date.split('/').reverse().join('-')).getTime() - new Date(a.date.split('/').reverse().join('-')).getTime()));
  };
